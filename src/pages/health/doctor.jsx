  ...
  return (
    <Section title={copy.title}>
      <div className="space-y-6">
        <p className="text-xl font-bold text-slate-700">{copy.firstLine}</p>
        <p className="text-xl font-bold text-slate-700">{copy.secondLine}</p>
      </div>

      <Tip>
        <ul className="list-disc pl-6 space-y-2">
          {copy.emergencyTip.map((item, index) => (
      ...
      <Accordion title={copy.familyTitle}>
        <ul className="list-circle pl-6 space-y-3">
          {copy.familyContent.map((item, index) => (

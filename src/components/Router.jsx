...
export default function Router({ page, onNav, language = 'ET', handleBack }) {
  ...
  if (page.startsWith('category.')) {
    ...
    return <CategoryPg categoryId={categoryId} onNav={onNav} language={language} handleBack={handleBack} />;
  }

  // 3. If it's an article ID (contains a dot) or a known route
  if (Pg) {
    return <Pg language={language} onNav={onNav} handleBack={handleBack} />;
  }

  // 4. FALLBACK: Generic Article rendering for metadata-only articles or missing files
  if (page.includes('.')) {
    return <ArticleFallback articleId={page} language={language} onNav={onNav} />;
  }

  // 5. Absolute fallback: Home
  const HomePg = PAGES['home'];
  return HomePg ? <HomePg onNav={onNav} language={language} /> : <div>Page not found: {page}</div>;
}

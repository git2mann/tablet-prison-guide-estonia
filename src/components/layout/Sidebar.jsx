  ...
  return (
    <nav style={navStyle}>
      <div style={{ padding:"16px 18px", display:"flex", alignItems:"center", gap:12, borderBottom:`1px solid ${t.border}`, minHeight:68 }}>
        <div style={{ width:38, height:38, borderRadius:10, background:t.accent, border:"none", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"1rem", flexShrink:0, color:t.accentText, fontWeight:800 }}>P</div>
        {showFull && (
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontWeight:700, fontSize:"1rem", color:t.text, letterSpacing:0.5 }}>Prison Guide</div>
            <div style={{ fontSize:"0.7rem", color:t.dim, letterSpacing:1, textTransform:"uppercase" }}>Estonia 2025</div>
          </div>
        )}
        {isTablet && (
      ...
      {showFull && (
        <div style={{ padding:"12px 18px", borderBottom:`1px solid ${t.border}`, background:t.accentDim }}>
          <div style={{ fontSize:"1.2rem", fontWeight:800, color:t.accent, letterSpacing:1, fontFamily:"monospace" }}>{timeStr}</div>
          <div style={{ fontSize:"0.8rem", color:t.dim, marginTop:2 }}>{dateStr}</div>
        </div>
      )}
            ...
            <div key={n.id}>
              <button
                ...
                style={{
                  ...
                  cursor:"pointer", color: isMain ? t.accent : t.dim, fontSize:"1rem",
                ...
                {showFull && hasSub && <span style={{ fontSize:"0.85rem", transform: isExp ? "rotate(180deg)" : "", transition:"0.2s", color:t.dim }}>▾</span>}
              </button>
                ...
                <div style={{ paddingLeft:32, marginBottom:6 }}>
                  {n.sub.map(s => (
                    <button key={s.id} onClick={() => onNav(s.id)} style={{
                      ...
                      color: active === s.id ? t.accent : t.dim, fontSize:"0.95rem",
      ...
      <div style={{ padding:"10px 8px", borderTop:`1px solid ${t.border}`, display:"flex", gap:6 }}>
        <button onClick={toggleTheme} style={{ flex:1, padding:12, background:"transparent", border:`1px solid ${t.border}`, borderRadius:8, color:t.dim, cursor:"pointer", fontSize:"1.1rem", fontFamily:"inherit", minHeight:48 }}>{isDark ? "☀️" : "🌙"}</button>
        {!isTablet && (
          <button onClick={onToggle} style={{ flex:1, padding:12, background:"transparent", border:`1px solid ${t.border}`, borderRadius:8, color:t.dim, cursor:"pointer", fontSize:"1.1rem", fontFamily:"inherit", minHeight:48 }}>{collapsed ? "›" : "‹"}</button>
        )}
      </div>
    </nav>
  );
